...
public:
	...
	// Scale up. Faster
	static void Scalar(UGFX::Graphic16& orig, UGFX::Graphic16& dest)
	{
	...
	// Scale Up. Fast
	static void NearestNeighbor(UGFX::Graphic16& orig, UGFX::Graphic16& dest)
	{
	...
	// Scale down. Slow
	static void Average(UGFX::Graphic16& orig, UGFX::Graphic16& dest) {
		const float origWidth = orig.width();
		const float origHeight = orig.height();
	...
	// Scale up. Slow
	static void Bilineal(UGFX::Graphic16& orig, UGFX::Graphic16& dest)
	{
		const float origWidth = orig.width();
		const float origHeight = orig.height();
...
private:
	inline static ESP_Color::Color getpixel(UGFX::Graphic16& image, uint16_t x, uint16_t y)
	{
		auto rgb = image.readPixel(x, y);
		return ESP_Color::Color(rgb);
	}

	inline static void putpixel(UGFX::Graphic16& image, uint16_t x, uint16_t y, ESP_Color::Color& color)
	{
